...
for(i in 1:10){
  sigma <- para[i]
  u <- rnorm(5000,0,sigma)
  v <- rnorm(5000,0,sigma)
  x <- sqrt(u^2+v^2)
  # histogram
  ...
  x <- seq(0,5*sigma,length.out=10000)
...
p1 <- 0.75
z <- rbinom(1000, 1, p1)
x <- z*rnorm(1000) + (1-z)*rnorm(1000,3,1)
...
for(i in 1:length(n)){
  sktests <- numeric(m)
  for(j in 1:m){
    x <- mvrnorm(n[i], mu=rep(0,3), diag(x=rep(1,3)))
...
for (i in 1:m) {
  ...
  sigma <-
    sample(c(1, 10),
           ...
           prob = c(1 - e, e))
  for(l in 1:n){
    x[l,] <- mvrnorm(1, mu <- rep(0, 3), diag(rep(sigma[l], 3)))
  }
...
btout <- boot(scor,statistic=theta,99)
boot.ci(btout,0.95)
```
...
for(i in 1:rep){
  n <- 50
  x <- mvrnorm(n,mu=c(0,0,0,0,0),Sigma=diag(c(1,1,1,1,1)))
  y <- mvrnorm(n,mu=c(0,0,0,0,0),Sigma=diag(c(2,2,2,2,2)))
  z <- rbind(x,y)
  sizes <- c(50,50)
  p1[1,i] <- NN_p(z,sizes)
...
for(i in 1:rep){
  n <- 50
  x <- mvrnorm(n,mu=c(3,3,3,3,3),Sigma=diag(c(1,1,1,1,1)))
  y <- 1/30 * x + mvrnorm(n,mu=c(0,0,0,0,0),Sigma=diag(c(9,9,9,9,9)))
  z <- rbind(x,y)
  sizes <- c(50,50)
  p1[2,i] <- NN_p(z,sizes)
...
# example 4
for(i in 1:rep) {
  x <- mvrnorm(10, mu = c(0, 0, 0, 0, 0), Sigma = diag(c(1, 1, 1, 1, 1)))
  y <- mvrnorm(90, mu = c(2, 2, 2, 2, 2), Sigma = diag(c(3, 3, 3, 3, 3)))
  z <- rbind(x, y)
  sizes <- c(10, 90)
  p1[4,i] <- NN_p(z, sizes)
...
for (i in 1:k){
  X1[i, ] <- g.chain(n, 0, 0)[,1]
  X2[i, ] <- g.chain(n, 0, 0)[,2]
}
